...
estimator_lookup <- tribble(
  ...
  "0.01",            "RieszNet,\n$\\lambda_{\\mathrm{Riesz}} = 0.01$",
  "0.1",             "RieszNet,\n$\\lambda_{\\mathrm{Riesz}} = 0.1$",
  "1",               "RieszNet,\n$\\lambda_{\\mathrm{Riesz}} = 1$",
  "10",              "RieszNet,\n$\\lambda_{\\mathrm{Riesz}} = 10$",
  "100",             "RieszNet,\n$\\lambda_{\\mathrm{Riesz}} = 100$",
...
riesz_weights <- ggplot(riesz_plot_data, aes(x = estimator, y = mse, group = 1)) +
  ...
  labs(
    x = "Estimator",
    y = "$\\mathrm{MSE}$",
    title = "RieszNet MSE with Varying $\\lambda_{\\mathrm{Riesz}}$"
  ) +
...
tmle_weights <- ggplot(tmle_plot_data, aes(x = tmle_weight, y = mse)) +
  ...
  labs(
    x = "$\\lambda_{\\mathrm{TMLE}}$",
    y = "$\\mathrm{MSE}$",
    title = "RieszNet MSE with Varying $\\lambda_{\\mathrm{TMLE}}$"
  ) +
  theme_classic(base_size = latex_base_size, base_family = latex_font_family) +
  theme(
