...
from datasets.datasets import ASEExperimentDataset
from models.neural_nets.dope_net import DOPENeuralNet
from models.neural_nets.functionals import AverageShiftEffect
...
def run_experiment():
    result_store = ResultStore("results/ase_plot_experiment/experiment.csv")
    for seed in range(1000):
        for model_type in MODEL_TYPES:
            for n_est_samples in N_EST_SAMPLES:
                if result_store.iteration_completed(
                    {"seed": seed, "model": model_type, "n_est_samples": n_est_samples}
                ):
                    continue
...
def _run_iteration(seed, model_type, n_est_samples):
    ...
    data = ASEExperimentDataset.simulate_dataset(n_samples=2 * n_est_samples)
    truth = data.truth
    data.create_folds(2)
    data_train, data_est = data.get_fit_and_test_folds(test_fold_index=1)

    functional = AverageShiftEffect(shift=1)
...
def _fit_model(model_type, functional, data_train, seed):
    model_config = ModelConfig(outcome_branch_type="s_learner")
    training_config = TrainingConfig()

    if model_type == "separate_nets":
        model = SeparateNeuralNets(
            moment_functional=functional, n_covariates=data_train.n_covariates, model_config=model_config
        )
        ...
        selected_representation_size = select_representation_size(data=data_train, functional=functional, seed=seed)
        model = DOPENeuralNet(
            moment_functional=functional,
            n_covariates=data_train.n_covariates,
            model_config=ModelConfig(
                shared_hidden_layers=[200, 200, selected_representation_size], outcome_branch_type="s_learner"
            ),
        )
        model.fit_outcome_branch(data=data_train, training_config=training_config)
        model.freeze_shared_trunk()
        ...
        model = RieszNet(
            moment_functional=functional, n_covariates=data_train.n_covariates + 1, model_config=model_config
        )
...
def select_representation_size(data, functional, seed):
    ...
    for representation_size in [1, 3, 10, 200]:
        ...
        model = DOPENeuralNet(
            ...
            model_config=ModelConfig(
                shared_hidden_layers=[200, 200, representation_size], outcome_branch_type="s_learner"
            ),
        )
